...
export const getCars = async (req, res, next) => {
  try {
    ...
    // Attach availability summary for each car (use instance method)
...
export const updateCar = async (req, res, next) => {
  try {
    ...
    // If new uploaded file present, remove old local file
    if (req.file) {
      if (car.image) {
      ...
      car.image = req.file.filename; // filename only
...
export const deleteCar = async (req, res, next) => {
  try {
    ...
    // Optionally, remove all bookings referencing this car (or mark them) - currently we leave bookings intact.
